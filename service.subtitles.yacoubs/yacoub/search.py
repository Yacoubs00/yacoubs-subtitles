...
def __get_last_results(core, meta):
    # FORCE FRESH SEARCH EVERY TIME - No caching
    core.logger.debug('Cache disabled - forcing fresh search')
    return ([], [])
...
def __apply_limit(core, all_results, meta):
    ...
    core.logger.debug('Results limit setting: %d' % limit)
    core.logger.debug('Total results before limit: %d' % len(all_results))
    ...
    core.logger.debug('Total results after limit: %d' % len(results[:limit]))
...
def __complete_search(core, results, meta):
    ...
    # Sort results by language alphabetically before displaying
    results.sort(key=lambda x: x.get('lang', ''))
    core.logger.debug('Results sorted by language: %d total' % len(results))
...
def search(core, params):
    # Force reload addon settings to get latest values
    try:
        core.kodi.reload_addon()
    except:
        pass
    ...
    # Get languages from Kodi - Priority order:
    # 1. params['languages'] - Kodi passes the user's configured subtitle languages here
    # 2. system settings - Backup method to read from Kodi settings
    # 3. Fallback to English if all else fails
    
    if 'languages' in params and params['languages']:
        # Primary method: Kodi sends the user's configured subtitle languages as a parameter
        meta.languages = __parse_languages(core, core.utils.unquote(params['languages']).split(','))
        core.logger.debug('Using languages from Kodi params: %s (raw: %s)' % (str(meta.languages), params['languages']))
    else:
        # Backup: Try to read from system settings
        system_languages = core.kodi.get_system_subtitle_languages()
        core.logger.debug('No languages in params. System languages detected: %s' % str(system_languages))
        
        if system_languages and len(system_languages) > 0:
            meta.languages = __parse_languages(core, system_languages)
            core.logger.debug('Using languages from system settings: %s' % str(meta.languages))
        else:
            # Ultimate fallback - read from Kodi's subtitle settings directly
            core.logger.debug('No languages found - addon will search for all available subtitles')
            meta.languages = []  # Empty means search all languages
    ...
    if len(results) > 0:
        core.logger.debug('Using cached results (%d results found). To force refresh, clear cache.' % len(results))
    else:
        core.logger.debug('No cached results found. Performing fresh search.')
    
    # Smart search: Skip providers based on content type
    smart_search = core.kodi.get_bool_setting('general', 'smart_search')
    ...
    for service_name in core.services:
        if len(results) > 0 and (__has_results(service_name, results) or service_name not in force_search):
            core.logger.debug('Skipping %s (already in cache)' % service_name)
            continue

        is_enabled = core.kodi.get_bool_setting(service_name, 'enabled')
        core.logger.debug('Provider %s: enabled=%s' % (service_name, is_enabled))
        
        if not is_enabled:
            core.logger.debug('Skipping %s (disabled in settings)' % service_name)
            continue
        
        # Smart search logic
        if smart_search:
            # YIFY: Movies only
            if service_name == 'yify' and meta.is_tvshow:
                core.logger.debug('Skipping %s (smart search: TV show, YIFY is movies only)' % service_name)
                continue
            
            # Witanime & Kitsunekko: TV shows/anime only
            if service_name in ['witanime', 'kitsunekko'] and not meta.is_tvshow:
                core.logger.debug('Skipping %s (smart search: movie, provider is for TV/anime only)' % service_name)
                continue

...
def get_system_subtitle_languages():
    ...
    try:
        # Method 1: Try to get preferred subtitle language from Kodi settings
        # This reads from Settings -> Player -> Language -> Preferred subtitle language
        subtitle_lang = xbmc.getInfoLabel("Skin.String(PreferredSubtitleLanguage)")
        if subtitle_lang and subtitle_lang != "":
            return [subtitle_lang]
        
        # Method 2: Get audio language from currently playing media
        try:
            player = xbmc.Player()
            if player.isPlaying():
                audio_lang = player.getAvailableAudioStreams()
                if audio_lang and len(audio_lang) > 0:
                    # Convert audio language to subtitle language preference
                    return [audio_lang[0]]
        except:
            pass

...
class A4kSubtitlesApi(object):
    def __init__(self, mocks=None):
        # Only set API mode when mocks are explicitly provided
        if mocks is not None and len(mocks) > 0:
            # Running in test mode with mocks
            api_mode = {
                'kodi': False,
                'xbmc': False,
                'xbmcaddon': False,
            ...
            # Running in real Kodi - ensure api_mode is NOT set
            if api_mode_env_name in os.environ:
                del os.environ[api_mode_env_name]
        
        self.core = importlib.import_module('yacoub.core')

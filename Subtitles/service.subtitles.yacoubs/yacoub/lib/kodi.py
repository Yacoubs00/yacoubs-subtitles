...
def reload_addon():
    """Force reload addon settings"""
    global addon
    addon = xbmcaddon.Addon()

addon = xbmcaddon.Addon('service.subtitles.yacoubs')
...
def get_system_subtitle_languages():
    """Get preferred subtitle languages from Kodi system settings - Added by Yacoub"""
    try:
        # Method 1: Read from Kodi's subtitle language settings
        # This is the proper way to get the user's preferred subtitle languages
        subtitle_languages = get_kodi_setting('subtitles.languages')
        if subtitle_languages and 'value' in subtitle_languages:
            langs = subtitle_languages['value']
            if isinstance(langs, list) and len(langs) > 0:
                return langs
        
        # Method 2: Try locale.subtitlelanguage setting
        subtitle_lang = get_kodi_setting('locale.subtitlelanguage')
        if subtitle_lang and 'value' in subtitle_lang:
            lang_value = subtitle_lang['value']
            if lang_value and lang_value != "":
                return [lang_value]
        
        # Method 3: Get system language as fallback
        system_lang = xbmc.getLanguage(xbmc.ISO_639_1, region=False)
        if system_lang and system_lang != "":
            return [system_lang]
            
        # Method 4: Ultimate fallback to English
        return ["en"]
